...
const Product = () => {
  ...
  // The CORS issue should be fixed now that the server is configured.
  ...
  const fetchProducts = async () => {
    ...
    setError(null);
    try {
      // Use a try-catch block for API calls to handle network and server errors gracefully.
  ...
  /**
   * Adds a new product to the database via a POST request to the API.
   * A '500 Internal Server Error' often indicates a mismatch between the
   * client-side JSON and the server's expected object model.
   * Ensure that the field names (`name`, `category`, `price`, `description`)
   * and their data types match exactly what is defined in your Spring Boot
   * `Product` class.
   */
  const handleAddProduct = async (e) => {
    ...
    try {
      // We are explicitly constructing the object to send to the server.
      // This helps ensure the structure is what the server expects.
      const productData = {
        name: formData.name,
      ...
      if (!response.ok) {
        // If the server responds with an error, the response status will not be in the 200 range.
  ...
  /**
   * Updates an existing product via a PUT request to the API.
   */
      ...
      case 'details':
        return (
          <div className="details-container">
            <div className="form-group">
              <label htmlFor="product-select">Select Product to View</label>
  ...
  return (
    <div className="product-management-app">
      ...
      <div className="button-group">
        <button onClick={() => { setMode('add'); setSelectedProductId(''); setFormData({ name: '', category: '', price: '', description: '' }); setError(null); }} className={`mode-button ${mode === 'add' ? 'active' : ''}`}><Plus size={20} /><span>Add New</span></button>
        <button onClick={() => { setMode('update'); setSelectedProductId(''); setFormData({ name: '', category: '', price: '', description: '' }); setError(null); }} className={`mode-button ${mode === 'update' ? 'active' : ''}`}><Edit size={20} /><span>Update</span></button>
        <button onClick={() => { setMode('details'); setSelectedProductId(''); setFormData({ name: '', category: '', price: '', description: '' }); setError(null); }} className={`mode-button ${mode === 'details' ? 'active' : ''}`}><Eye size={20} /><span>Details</span></button>
        <button onClick={() => { setMode('delete'); setSelectedProductId(''); setFormData({ name: '', category: '', price: '', description: '' }); setError(null); }} className={`mode-button ${mode === 'delete' ? 'active' : ''}`}><Trash2 size={20} /><span>Delete</span></button>
      </div>
